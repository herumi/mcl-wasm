...
node -e "mcl = require('../dist'); mcl.init().then(() => console.log(mcl))" > ${CASE_NAME}/exported-fields.dump
node -e "mcl = require('../dist'); mcl.init().then(() => console.log(Object.keys(mcl).sort()))" > ${CASE_NAME}/exported-field-names.dump
