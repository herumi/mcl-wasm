...
let g_total:number = 0
let g_mallocPtr:{[key:number]:number}={}
/** @internal */
export const _malloc = (size: number): number => {
  const p = mod._mclBnMalloc(size)
  g_total += size
  g_mallocPtr[p] = size
  return p
...
export const _free = (pos: number): void => {
  const size = g_mallocPtr[pos]
  if (size === 0) {
    console.log(`bad free ${pos}`)
    return
  }
  g_total -= size
  delete g_mallocPtr[pos]
  mod._mclBnFree(pos)
}
